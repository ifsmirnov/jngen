...
BOOST_AUTO_TEST_CASE(antihashset) {
    ...
    Set s1;
    BOOST_CHECK(check(s1, rnda.antiUnorderedSetGcc4(1000)));

    Set s2;
    s2.max_load_factor(0.4);
    BOOST_CHECK(check(s2, rnda.antiUnorderedSetGcc4(1234, 0.4)));

    Set s3;
    s3.reserve(5000);
    BOOST_CHECK(check(s3, rnda.antiUnorderedSetWithReserveGcc4(5000)));
    ...
    BOOST_CHECK(check(s4, rnda.antiUnorderedSetWithReserveGcc4(3141, 0.31)));
    */
#else
